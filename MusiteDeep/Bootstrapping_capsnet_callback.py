...
def bootStrapping_allneg_continue_keras2(trainfile,valfile=None,srate=0.8,
  ...
  for I in range(nb_epoch1):
    ...
    for t in range(nclass):
        ...
        if t==0:
            models=Capsnet_main(trainX=trainX1,trainY=trainY1,valX=valX1,valY=valY1,nb_classes=nb_classes,nb_epoch=nb_epoch2,earlystop=earlystop,weights=inputweights,compiletimes=t,lr=0.001,batch_size=1000,lam_recon=lam_recon,routings=3,class_weight=None,modeltype=model,predict=predict,outputweights=outputweights,monitor_file=monitor_file_name,save_best_only=save_best_only,load_average_weight=load_average_weight)
        else:
            models=Capsnet_main(trainX=trainX1,trainY=trainY1,valX=valX1,valY=valY1,nb_classes=nb_classes,nb_epoch=nb_epoch2,earlystop=earlystop,weights=inputweights,compiletimes=t,compilemodels=models,lr=0.001,batch_size=1000,lam_recon=lam_recon,routings=3,class_weight=None,modeltype=model,predict=predict,outputweights=outputweights,monitor_file=monitor_file_name,save_best_only=save_best_only,load_average_weight=load_average_weight)
        
        #save weights already does in callbacks in capsnet_main
