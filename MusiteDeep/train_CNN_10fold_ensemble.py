...
import numpy as np
import argparse
from Bootstrapping_multiCNN_callback import bootStrapping_allneg_continue_keras2
...
if __name__ == "__main__":
    ...
    parser.add_argument('-checkpointweights',  dest='checkpointweights', type=str, help='prefix of the intermediate checkpoint files.', required=False, default=None)
    parser.add_argument('-save_best_only', dest='save_best_only',action='store_true',help="save the best models in each iteration")
    ...
    ensure_dir(args.outputprefix)
    ensure_dir(args.checkpointweights)
    
    outputmodel = args.outputprefix+str("_HDF5model")
    outputparameter = args.outputprefix+str("_parameters")
    ...
    try:
       ...
       output.write("%d\t%d\t%s\tgeneral\t%d\t%d" % (args.nclass,args.window,args.residues,codemode,nb_classes))
    
    start = timeit.default_timer()
    allfrag,folds=preprocess_data(args.inputfile,args.window,residues)
    for i,(train_indices,val_indices) in enumerate(folds):
        for bt in range(args.nclass):
            checkpointoutput=args.checkpointweights+"_fold"+str(i)+"_nclass"+str(bt)
